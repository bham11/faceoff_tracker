...
class HockeyWindow(tk.Tk):
    ...
    def __init__(self, database):
        ...
        self.faceoff_input_frame = FaceoffInputFrame(self, database, self.game_info_frame)
...
class FaceoffInputFrame(tk.LabelFrame):
    ...
    def add_FO(self,e):
        ...
        if self.result.get().upper() not in ['W', 'L']:
            ...
            self.database.insert_faceoff(self.options.what_period.get(), self.husky.get(), self.opp.get(), self.options.strength.get(),
                                 formatted_zone, self.result.get().upper())
...
class StatsFrame(tk.LabelFrame):
    def __init__(self, container, database):
        ...
        self.filter_first_period = tk.Radiobutton(master=self, text="1rst", value="1rst", variable=self.filter_periods)
        self.filter_second_period = tk.Radiobutton(master=self, text="2nd", value="2nd", variable=self.filter_periods)
        self.filter_third_period = tk.Radiobutton(master=self, text="3rd", value="3rd", variable=self.filter_periods)
        self.filter_ot = tk.Radiobutton(master=self, text="OT", value="OT", variable=self.filter_periods)
        self.filter_all_periods = tk.Radiobutton(master=self, text="All Per", value="all per", variable=self.filter_periods)
        self.filter_strength = tk.StringVar(value="all str")
        self.filter_even = tk.Radiobutton(master=self, text="Even", value="even", variable=self.filter_strength)
        self.filter_pk = tk.Radiobutton(master=self, text="PK", value="pk", variable=self.filter_strength)
        self.filter_pp = tk.Radiobutton(master=self, text="PP", value="pp", variable=self.filter_strength)
        self.filter_all_strengths = tk.Radiobutton(master=self, text="All Strs", value="all str", variable=self.filter_strength)
        self.give_all_ops = tk.StringVar(value= 'not op groups')
        self.by_all_ops = tk.Radiobutton(master=self, text= "By All Ops", value= 'by_all_ops', variable= self.give_all_ops)
        self.default_by_ops = tk.Radiobutton(master=self, text="Normal Ops", value='not op groups', variable=self.give_all_ops)
    ...
    def valid_stats_frame_opp(self, e):
        if not self.opp_look_up.get().isdigit() and not self.opp_look_up.get() == "":
            self.stats_log.insert("1.0", "Please input a valid opponent jersey number\n")
    ...
    def display_query(self):
        ...
        query = self.database.build_hockey_query(table_name='hockey_faceoff_data_table', husky=husky_val,
    ...
    def display_database(self):
        ...
        opp_val = self.opp_look_up.get()
        # def build_hockey_query(table_name, husky=None,opp=None, period=None, strength=None)
        query = self.database.build_hockey_query_db(table_name='hockey_faceoff_data_table', husky=husky_val,
                                      opp=opp_val, period=per_val, strength=strength_val, zone=zone_val)
        self.stats_log.delete("1.0", END)
        display_table = self.database.return_query_df(query)
