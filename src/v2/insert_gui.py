...
class FileFrame(tk.LabelFrame):
    def __init__(self, container):
        ...
        self.opponent_box = ttk.Combobox(
            master=self, state="readonly", values=OPPONENTS, height=5
        )
        self.select_opponent = tk.Label(master=self, text="Select Opponent:")
        self.scrollbar = Scrollbar(master=self, orient=HORIZONTAL)
        self.display_file_name = tk.Entry(
            master=self, xscrollcommand=self.scrollbar.set
        )
        self.insert_filename = tk.Entry(master=self)
        self.game_file_name = tk.Button(
            master=self, text="Select Game", command=self.get_filename
        )
        self.send_button = tk.Button(
            master=self, text="Insert Data", command=self.send_game_data
        )
...
class QueryFrame(tk.LabelFrame):
    def __init__(self, container):
        ...
        self.log_display = tkscrolled.ScrolledText(
            master=self,
            wrap="word",
        )
        self.view_button = tk.Button(
            master=self, text="View Data", command=self.get_game_data
        )
        self.group_by_opp = tk.BooleanVar(value=False)
        self.group_by_opp_radio = tk.Radiobutton(
            master=self, text="by opp", value=True, variable=self.group_by_opp
        )
        self.group_by_strength = tk.BooleanVar(value=False)
        self.group_by_strength_radio = tk.Radiobutton(
            master=self, text="by strength", value=True, variable=self.group_by_strength
        )
        self.group_by_period = tk.BooleanVar(value=False)
        self.group_by_period_radio = tk.Radiobutton(
            master=self, text="by period", value=True, variable=self.group_by_period
        )
        self.group_by_zone = tk.BooleanVar(value=False)
        self.group_by_zone_radio = tk.Radiobutton(
            master=self, text="by zone", value=True, variable=self.group_by_zone
        )
    ...
    def get_game_data(self):
        ...
        display_table = select_opponent_data(
            "2023-2024/production.db", opponent, add_fields, groups, filters
        )
